...
def inicializar_archivo():
    ...
    if not os.path.exists(ARCHIVO):
        with open(ARCHIVO, mode='w', newline='') as archivo: # 'w' para escribir, 'newline' para evitar lineas en blanco y with open para cerrar el archivo automaticamente
            ...
            writer.writeheader()
            writer.writerows([ 
                {'nombre': 'Huevos', 'precio': 120.5, 'cantidad': 30},
                {'nombre': 'Lechuga', 'precio': 80.0, 'cantidad': 77},
                {'nombre': 'Tomate', 'precio': 150.0, 'cantidad': 20}
            ])
...
with open(ARCHIVO, mode='r', newline='') as archivo:
    ...
    for fila in lector:
        productos.append(fila)  # cada fila es un diccionario
...
encontrado = False  # bandera de búsqueda exitosa
